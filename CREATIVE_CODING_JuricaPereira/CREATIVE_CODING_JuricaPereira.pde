...
final int Width = 800; //width
final int Height = 800;//height
...
void draw(){
  ...
  for(int i =0; i<day_number; i++){
    ...
    float direction = map(d.day, 1, day_number,0 , TWO_PI);
  ...
  if(shown != null){
    ...
    Xtip = lerp(Xtip, x1, 0.15);
    Ytip = lerp(Ytip, y1, 0.15);
  ...
  if(shown != null){
    ...
    trackVol = lerp(trackVol, targetVol, 0.1);
    song.amp(trackVol);
    
    float targetSpeed = 1 + map(shown.intensity, 1, 10, 0 , 0.2);
    trackSpeed = lerp(trackSpeed, targetSpeed, 0.1);
    ...
    trackVol = lerp(trackVol, 0.2,0.05);
    song.amp(trackVol);
    
    trackSpeed = lerp(trackSpeed, 1, 0.05);
    song.rate(trackSpeed);
  }
}
...
boolean isMouseOverBar(DearData d, float direction, float intensity_bar){
  ...
  float a1 = map(d.day - .5, 1 , day_number, 0, TWO_PI);
  float a2 = map(d.day + 0.5, 1, day_number, 0, TWO_PI);
...
color getColorForEmotion(String e){
  ...
  if(e.equals("Anxiety")) return color(300,80,90);
